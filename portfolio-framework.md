...
**Play authoring lock, June 11, 2026; audited June 24:** `/play` media is a Framer-authorable content surface, not hardcoded decoration. Target source of truth is the user-managed `Play Archive` CMS collection (`EySMRbI2N`) for title, order, image/poster, video, stroke, and drawer/sidebar `Content`. Current code can only read CMS live when Framer ships an importable runtime collection module; otherwise it falls back to `Archive Items` / `archiveItems` or the baked Play Archive snapshot. Do not remove the authorable array, the CMS reader, or replace the archive with unrelated static JSX/media layers. Until a complete runtime CMS bridge is installed, publish/rebake after CMS content changes before expecting the public `/play` route to match CMS exactly. The earlier `/play-consolidation-draft` route is no longer in the current Framer page inventory; create a fresh draft/design page for future interaction refactors before promoting to production.
...
The order is a deliberate argument. Each visible Home card earns its slot and reinforces the one before it. Current Framer Home uses `HomeSelectedWorkGrid.tsx` to render a CMS-backed selected-work set: `Is Homepage = true`, ordered by `Sorting Number`, limited to 6 records, with direct `/case-studies/{slug}` links, image/video thumbnail handling, thumbnail strokes, and Category 1/2/3 tag pills. Pill type is 13px on desktop, 12px on tablet, and 11px at the mobile/single-column breakpoint.
...
3. **Work zone** — current Framer implementation uses `HomeSelectedWorkGrid.tsx` for the six-item selected-work section. It preserves the intended card visuals while using direct `/case-studies/[slug]` anchors, image/video thumbnail handling, responsive Category 1/2/3 tag pills, and a CMS/default data fallback. The `VIEW ALL` CTA links to `/index`. Do not reintroduce the broken native Home `AllProjects` / `CaseStudy` grid or a hydration `LinkRepair` as the primary Home mechanism.
