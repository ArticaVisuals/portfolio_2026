...
**For:** Micah Hoang
**Version:** 1.2 — May 22 Framer route audit reflected. CMS is still 15 real projects. Home is a six-item CMS-limited selected-work query. `/info`, `/play`, `/index`, `/case-studies`, `/contact`, and `/case-studies/airpods` are published Framer routes; `/play-2` and `/playground-scroll-draft` remain unpublished drafts.
**Date:** May 2026
**Last live route audit:** May 22, 2026. Published/staging URL `https://khaki-ship-257706.framer.app`. `https://micahhoang.info` still served the Cargo site during the audit, so treat the Framer URL as the current build surface until domain cutover. Current-state companion: `framer-current-state.md`.
...
Current implementation note (May 22, 2026): Framer code file `rgAZFOv` owns the `/index` archive component, and the repo mirror has been reconciled to the current direction. `IndexPage.tsx` now tries to load Framer's generated `All Projects` CMS module directly when `useCMS=true`, then falls back to the legacy window registry, then manual `projects`, then `DEFAULT_PROJECTS`. The May 22 browser audit confirmed the CMS module resources load on `/index`, but the window registry is empty and the visible industry labels remain simplified. Do not assume long CMS labels or a populated Registrar path without rechecking the live page.
