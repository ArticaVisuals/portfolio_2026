...
**Play authoring lock, June 11, 2026; updated June 24:** `/play` media is a Framer-authorable content surface, not hardcoded decoration. The live source now prefers the user-managed `Play Archive` CMS collection (`EySMRbI2N`) for title, order, image/poster, video, stroke, and drawer/sidebar `Content`. The production wrapper `Play.tsx` (`PN1RVOf`) must still expose `Archive Items` / `archiveItems` and pass those managed items into `ArchivePlayground.tsx` (`QNpkYp5`) as the canvas/editor fallback and rollback surface. Do not remove the authorable array, the CMS reader, or replace the archive with static JSX/media layers. The earlier `/play-consolidation-draft` route is no longer in the current Framer page inventory; create a fresh draft/design page for future interaction refactors before promoting to production.
