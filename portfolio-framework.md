...
│   ├── Header: "INDEX" title (in a SectionHero above the code component) + fixed floating List/Grid view toggle
│   ├── A/B comparison route: `/index-inline-toggle-test` (`VdRy9MV8k`) mirrors this page but swaps the floating toggle for an original-template-style inline `GRID / LIST` control
...
- ~~**Index page layout.**~~ ✅ Resolved (May 6 update; A/B route added May 16). Canonical `/index` is `u2LOaBT5q`; the old duplicate `yKKOMVNs6` is gone. `/index` keeps the fixed bottom-left List/Grid toggle with two equal-width buttons and black/ink text for both active and inactive states; on mobile it pins to bottom-center. `/index-inline-toggle-test` (`VdRy9MV8k`) is a non-destructive comparison route that hides the floating toggle only on that duplicate and renders an uppercase `GRID / LIST` control at the top-right of the project content, with the active option muted like the Jacob Turner template. List view (default): year-grouped projects with taxonomy filters. Taxonomy follows Figma node `32:7531` but uses a flexible six-column grid: Discipline label/value cols 1/2, Industry label/value cols 3/4, Year label/value cols 5/6. The List year-group wrapper shares that 6-col grid; inside it, list rows use a 5-col inner grid (title 1/span 2, discipline 3/span 2, industry 5/span 1). Industry is never hidden; Discipline/Industry truncate with ellipses on desktop/tablet and reflow on mobile. `List Type` A/B: `Standard` keeps large year + 22px title; `Mono 13` makes all list typography 13px uppercase mono. `List Hover` A/B: `Flip` is default (title-only upward flip mirroring `ViewProject` reference `node=L21w7Xq1z`); `Highlight` preserves the older row background hover. The second group is `Industry`, not `Origin`. Grid view renders project-driven cards; the unfiltered `Case Studies Filter` fallback was removed. Discipline / Industry / Year nav values are derived dynamically from the in-scope projects; the previous "eight canonical Discipline labels" lock is not enforced in code. Do not expose 3D mode in `/index` unless Micah explicitly asks; `WorldGridTest.tsx` is unrouted.
