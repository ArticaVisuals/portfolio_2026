...
3. **Work zone** — current Framer implementation uses `HomeSelectedWorkGrid.tsx` for the six-item selected-work section. It preserves the intended card visuals while using direct `/case-studies/[slug]` anchors, CMS image/video thumbnail handling, responsive Category 1/2/3 tag pills, and no baked project fallback rows. The `VIEW ALL` CTA links to `/index`. Do not reintroduce the broken native Home `AllProjects` / `CaseStudy` grid or a hydration `LinkRepair` as the primary Home mechanism.
