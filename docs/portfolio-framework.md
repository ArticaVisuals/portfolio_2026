...
**Play authoring lock, June 11, 2026; CMS-only update June 25:** `/play` media is a Framer CMS content surface, not hardcoded decoration. The user-managed `Play Archive` CMS collection (`EySMRbI2N`) is the live source of truth for title, order, image/poster, optional video, stroke, and drawer/sidebar `Content`; if a value is not in CMS, the published page must not invent it. `ArchivePlayground.tsx` reads live data from the `PlayArchiveRegistrar` registry first, then a generated `EySMRbI2N` CMS module if Framer emits one, then empty state. `Archive Items` / `archiveItems` remains only a protected canvas/rollback authoring surface. Do not reintroduce baked Play arrays, Cargo/default snapshots, or detached media layers. Long-term editing workflow is documented in `play-cms-workflow.md`.
...
The order is a deliberate argument. Each visible Home card earns its slot and reinforces the one before it. Current Framer Home uses `HomeSelectedWorkGrid.tsx` to render a CMS-backed selected-work set: `Is Homepage = true`, ordered by `Sorting Number`, limited to 6 records, with direct `/case-studies/{slug}` links, video-first thumbnail handling, thumbnail strokes, and Category 1/2/3 tag pills. CMS `Thumbnail Video` is preferred whenever present; CMS `Thumbnail` is the poster/fallback image. Selected-work number/title text is the original 13px uppercase mono treatment. Pill type is 13px on desktop, 12px on tablet, and 11px at the mobile/single-column breakpoint.
...
Current implementation note (June 11, 2026; motion ease consolidation updated June 18; media priority corrected July 20): Framer code file `rgAZFOv` owns the `/index` archive component, and the current Framer file is source of truth when the local mirror drifts. Keep responsive/index styling, inline-toggle alignment, rule/line timing, masked slide-in reveals for large list/grid year/title text, mono fade-in reveals for taxonomy/nav/meta text, full-list page-level reveal timing, `GridMediaFrame` thumbnail fades, and direct `.idx-grid-card-media > img/video` hover/focus scale in `IndexPage.tsx` rather than hidden companion helpers. Index motion should stay on Snappy `cubic-bezier(0.16, 1, 0.3, 1)` for decelerating ease-out reveals, fades, masks, hover transforms, and media scale; Smooth `cubic-bezier(0.12, 0.23, 0.5, 1)` for symmetric in/out rule draws, small UI state transitions, and prior bare `ease` transitions. Legacy decel curves such as `cubic-bezier(0.22, 1, 0.36, 1)` map to Snappy; legacy symmetric curves such as `cubic-bezier(0.33, 0, 0.67, 1)` map to Smooth. The broader coded-ease set also reserves Springy `cubic-bezier(0.25, 1, 0.5, 1)` for Image Trail's punchy gestural feel in the live Framer source; do not broaden it into index/list motion. The removed `IndexThumbnailVideoFallback.tsx` hardcoded per-project media and should not be recreated.

Thumbnail media policy is: `Thumbnail Video` wins over `Thumbnail`; `Thumbnail` acts as poster/fallback. With `useCMS=true`, `IndexPage.tsx` waits for and renders only Framer's generated `All Projects` CMS module; it intentionally does not use manual `projects`, baked defaults, `ProjectRegistrar`/window registry rows, hardcoded media URLs, project-specific rules, or any in-code project snapshot in CMS mode. There are no project-specific thumbnail/video allow-lists, deny-lists, or asset-ID rules; edit CMS `Thumbnail` and `Thumbnail Video` fields instead. The older `Thumbnail Video Link` text field (`WG62tRjG8`) is retired; do not wire new thumbnail-video behavior to it. Publish/redeploy Framer after editing code files or CMS File/image fields so the public CMS bundle refreshes. `IndexPage.tsx` keeps direct `.idx-grid-card-media > img/video` children on the same hover/focus `scale(1.02)` path and resets them under reduced motion.

Grid hover guardrail: the mounted Figma-grid wrapper (`IndexPageGridPreview.tsx`, `LgIzFjJ`) clips the title frame at 13px and stacks the normal title plus `VIEW PROJECT` with a 5px gap. Its hover/focus override must remain `translate3d(0, -18px, 0)`; `-13px` clips the `VIEW PROJECT` label.
...
3. **Work zone** — current Framer implementation uses `HomeSelectedWorkGrid.tsx` for the six-item selected-work section. It preserves the intended card visuals while using direct `/case-studies/[slug]` anchors, CMS video-first thumbnail handling, responsive Category 1/2/3 tag pills, original 13px number/title text, and no baked project fallback rows. The `VIEW ALL` CTA links to `/index`. Do not reintroduce the broken native Home `AllProjects` / `CaseStudy` grid or a hydration `LinkRepair` as the primary Home mechanism.
4. **About zone** — current Framer Home uses a portrait plus two columns of bio copy and a `read more` link to `/info`. Preserve the Framer layer contract: the `Button Wrapper` owns the `/info` link, while the visible `READ MORE` flip text is detached native `TextLinkBlack` content, not a linked `Text Link` component. This avoids Framer nested-link optimizer warnings. As of July 15, 2026, `HomeSelectedWorkGrid.tsx` (`FecepLS`) owns the restored portrait zoom and `READ MORE` flip CSS.
...
- ~~**Homepage layout.**~~ ✅ Resolved. Current Framer Home work section is `HomeSelectedWorkGrid.tsx`, rendering six selected projects from CMS order: Gaia, AirPods Pro 3, Peak Energy, Simon & Schuster, Motion Connect 2025, and National Park Playing Cards. The grid has no baked fallback rows, prefers `Thumbnail Video` over CMS poster images, and keeps the selected-work number/title style at 13px uppercase mono. This replaced the broken native Home selected-work grid whose CMS bindings collapsed to AirPods. Current hero is 60vh, not full viewport.
