...
- **`ParagraphPrettyWrap` helper added for site-wide paragraph orphan control.**
  Framer codeFile `EjvkJhv` scans rendered Framer text and applies native
  `text-wrap: pretty` only to paragraph-size text at 23px and under. It excludes
  header/nav/footer/UI controls, respects `white-space: nowrap`, and supports a
  `data-mh-pretty-ignore` opt-out. The helper is singleton-aware so multiple
  mounts share one observer. `PageTransition.tsx` (`gmalnRr`) imports and renders
  it before the v7.12 runtime; `ResumeAssetHost.tsx` (`xDqfenf`) carries a
  self-contained fallback from the hidden Footer host so routes without
  PageTransition still get covered.
- **Verification:** Framer MCP accepted `ParagraphPrettyWrap`, `PageTransition`,
  and `ResumeAssetHost` with empty typecheck results. After the 2026-07-21
  12:36 PDT publish, production `/info` loaded the Footer fallback and marked
  `29` nodes; production `/case-studies/airpods` loaded the PageTransition path
  and marked `6` nodes. Both routes had `0` marked nodes above the then-active
  paragraph-size threshold, `0` marked nodes inside excluded regions, and `0`
  marked nodes missing computed `text-wrap: pretty`. Home and `/play` loaded the
  helper style tag and correctly marked `0` nodes because no qualifying
  paragraph text at 23px and under was present outside excluded UI regions.
- **Gaia follow-up:** a visible widow on `/case-studies/gaia` was traced to a
  `22px` intro/body paragraph that the original under-20px rule skipped. The
  helper and Footer fallback now use an inclusive `<= 23px` threshold; a live
  injection check changed that paragraph's final line from `world.` to
  `living world.` before publish.
- **2026-07-22 pre-paint update:** `ParagraphPrettyWrap` and the
  `ResumeAssetHost` fallback now render an inline preload `<style>` plus a
  singleton inline preload script. The preload script uses the same 23px,
  min-word, exclusion, own-text, block-child, and whitespace checks as the
  hydrated controller, starts observing the document immediately, and scans
...
- **`/play` load-in and grid performance pass published.** `ArchivePlayground.tsx`
  now requests grid thumbnails at `scale-down-to=720`, leaves grid videos at
  `preload="none"`, memoizes media frames, skips duplicate aspect measurement
  when CMS dimensions are known, and delays the generated-CMS fallback only long
  enough to confirm the registrar bridge is empty. `Play.tsx` now exposes the
  production video budget controls and ramps autoplay from `8` initial
  concurrent videos to `16` after `2400ms`.
- **`/play` random-gap fix.** The wide desktop lattice caps row/column gutters at
  `56px` (`190px` cell + `56px` gap = `246px` step) and renders up to `16`
  columns × `12` rows so large viewports stay covered. Grid media wrappers no
  longer sit at `opacity:0` while waiting for per-image/per-video load callbacks;
  that fade is kept for detail drawer media only. Visible/near-visible grid
  images load eagerly, while offscreen buffer cells remain lazy.
- **Published QA on `https://micahhoang.com/play`.** At `2560×1440`, after drift:
  `0` no-media cells, `0` hidden media wrappers, consistent `246px` row/column
  stepping, `720px` image requests, grid videos still `preload="none"`, and no
...
- **Controller import pin refreshed.** `CaseStudyControllers.tsx` now imports the
  ...
  `CaseStudyLightbox-yOYpGN.js@aAHiy1bZV8EEshPXW8Zh`; live Framer typecheck
...
| `CaseStudyJustifiedMediaGrid.tsx` | `c0iPrbN` | Canonical bespoke case-study justified media grid helper, with per-item fit/stroke and poster support. |
| `FixedHeightMediaRows.tsx` | `IthLMt_` | Deprecated bridge only; legacy instances delegate into `CaseStudyJustifiedMediaGrid.tsx`. Do not create new instances. |
...
June 15, 2026 archive thumbnail video update, corrected July 20: the live `/index` `IndexPage` instance reads `thumbnailVideoFieldIds="SvOqFqdby"`, so the `Thumbnail Video` File upload is the only active CMS thumbnail-video source. The older `Thumbnail Video Link` text field (`WG62tRjG8`) is retired and should not be used as a fallback. Thumbnail media policy is: `Thumbnail Video` wins over `Thumbnail`; `Thumbnail` is poster/fallback. The hidden `CmsLink` collection list remains mounted so Framer's generated CMS module is available, but `IndexPage` now waits for and renders only the generated CMS module in CMS mode. The index media pipeline has no project-specific thumbnail/video allow-list, deny-list, or asset-ID rule; it renders the CMS `Thumbnail Video` and `Thumbnail` fields as provided, and if the CMS module has not produced rows yet the grid stays loading/empty rather than using registry/manual/baked media. The existing `CaseStudyThumbnailStrokeStyles.tsx` instance on `/index` (`szF9sZNWA`) remains configured with `syncThumbnailVideos=true`, `videoFieldId="SvOqFqdby"`, and `slugFieldId="pdXVG_fBO"` as a backup overlay path. The published site needs a Framer publish/redeploy before code-file and canvas/CMS field changes appear in the generated CMS bundle. `IndexThumbnailVideoFallback.tsx` (`wvucZCT`) and its hidden node (`R9kqDJO7t`) were deleted from Framer; do not recreate a hardcoded per-project fallback helper.

July 20, 2026 index regression guard: after any small `/index` tweak, scan `IndexPage.tsx` and `IndexPageGridPreview.tsx` for `DEFAULT_PROJECTS`, `fromRegistry`, `registeredProjects`, `__articaIndexProjectsRegistry`, `ProjectRegistrar`, renderable `framerusercontent.com/images` or `/assets` URLs, and `translate3d(0, -13px`. None should be present in live `/index` source. Confirm the positive path still has `useCMS ? (fromCMSModule ?? []) : (fromProps ?? [])`, `projects={useCMS ? [] : projects}`, and `translate3d(0, -18px, 0)`. After Framer publish, browser-check a fresh `/index` load: Wolff Olins and Independent Lens must use their CMS `Thumbnail` URLs, video-backed projects must render `<video>` before image with CMS `Thumbnail` as poster/fallback, and hovering a grid card must fully reveal `VIEW PROJECT`.

June 1, 2026 CMS bridge note, revised July 20: `/index` includes a hidden-by-position CMS bridge layer, `CmsLink` (`AwTGGhR7I`), containing the `AllProject` collection item. Keep this collection layer **visible/mounted** in Framer's layer panel, but fixed off-canvas at `left="-202px"`, `width="1px"`, `height="1px"`, `opacity="0"`, `overflow="hidden"`, and locked, because it keeps Framer's generated CMS module available to `IndexPage`. If `ProjectRegistrar` (`I063adJ_h`) remains mounted for legacy reasons, it must not be used as a `/index` render fallback.

Data priority with `useCMS=true` is:

1. Direct generated CMS module scan for `All Projects`

In CMS mode, `IndexPage.tsx` intentionally falls through to an empty array rather than manual props, `ProjectRegistrar`/window registry rows, hardcoded media, or any in-code project snapshot, so stale fallback labels and thumbnails cannot appear as live CMS content. There is no `DEFAULT_PROJECTS` fallback.
...
- project-specific media/gallery helpers such as `TypldnProcessGallery.tsx` and `SeekTruthCargoSlideshow.tsx` (note: `FixedHeightMediaRows.tsx` is now only a deprecated bridge into `CaseStudyJustifiedMediaGrid.tsx`; `SimonSchusterGuidelinesCarousel.tsx` / code file `tYFZCey` is **no longer project-specific** — it is now the reusable `ImageCarousel`; recycle it for new galleries rather than writing a new helper. See "Reusable Image Carousel" in Code Components.)
...
Use `node code/tools/check-thumbnail-stroke-resolver.mjs` as the quick local regression guard for the stroke helper. It confirms the helper still uses Light Gray `#979797`, the CMS `Thumbnail Stroke` field, the current `module.r` CMS export shape, the legacy `module.a` fallback, and the shared resolver call.
