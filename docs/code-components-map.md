...
| `ResumeAssetHost` | `xDqfenf` | Footer compatibility host mounted across published routes. Preserves the `resumeFile` prop/invisible div that Footer expects, carries the `ParagraphPrettyWrap` fallback for pages without PageTransition, and mounts `NavigationScrollGuard` as the guaranteed global Lenis host. |
