...
| `ArchivePlayground` | `QNpkYp5` | `/play` archive renderer: live mode renders Play Archive CMS rows only (`PlayArchiveRegistrar` registry first, generated `EySMRbI2N` module second, otherwise empty). Owns grid + media smoothing + nav passthrough + close timing, and the detail drawer: **off-black** title, `/Text Gray` description, **divider directly under the title**, and a **Link-gated CTA** (renders only when CMS `Link` is set; label from `Link Title`, nav-mono `→` glyph + hover roll). Two-column on wide panels (title+divider | description+CTA, CTA aligned to body copy), single left-aligned stack on the smallest breakpoint. Poster-less video cells render nothing (no broken-image icon) when outside the concurrent-video budget. |
