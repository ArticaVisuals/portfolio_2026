...
function isCandidate(element: HTMLElement, maxFontSize: number) {
    ...
    if (element.closest(DEFAULT_EXCLUDE_SELECTOR)) return false
...
[${ATTR}="true"] {
    ...
    const run = () => {
        ...
        document.body.querySelectorAll(TEXT_SELECTOR).forEach((element) => {
            ...
            if (isCandidate(element, limit)) element.setAttribute(ATTR, "true")
            else if (element.getAttribute(ATTR) === "true") element.removeAttribute(ATTR)
        })
    }
    state.run = run
    run()
    ...
    return () => {
        ...
        document.querySelectorAll(`[${ATTR}="true"]`).forEach((element) => element.removeAttribute(ATTR))
    }
}
